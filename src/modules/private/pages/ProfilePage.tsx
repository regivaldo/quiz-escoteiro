      ...
      try {
        ...
        const historyData = querySnapshot.docs.map((doc) => {
          ...
          return {
            ...
            date:
              data.respondedAt?.toDate().toLocaleDateString('pt-BR', {
                day: 'numeric',
                month: 'long',
                year: 'numeric',
              }) || 'Data desconhecida',
    ...
    <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16">
      <div className="max-w-4xl mx-auto flex flex-col gap-10">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-4xl font-black tracking-tighter text-text">Meu Perfil</h2>
          <button
            className="flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-red-500/10 px-4 py-2 text-sm font-bold text-red-500 transition-colors hover:bg-red-500/20"
        ...
        <h3 className="text-2xl font-bold tracking-tight text-text">Minhas Estatísticas</h3>

        {/* Stats Grid */}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-1 lg:grid-cols-2">
          {stats.map((stat, index) => (
            <div key={index} className="flex flex-col gap-2 rounded-xl border border-gray-200 bg-white p-6">
              <div className="flex items-center gap-2 text-gray-600">
                <span className={`text-${stat.color} text-xl`}>{stat.icon}</span>
        ...
        <h3 className="text-2xl font-bold tracking-tight text-text">Histórico de Quizzes</h3>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left font-medium text-gray-500" scope="col">
                    Nome do Quiz
                  </th>
                  <th className="px-6 py-3 text-left font-medium text-gray-500" scope="col">
                    Pontuação
              ...
              <tbody className="divide-y divide-gray-200">
                {quizHistory.length > 0 ? (
                  quizHistory.map((quiz) => (
                    <tr key={quiz.id}>
                      <td className="whitespace-nowrap px-6 py-4 font-medium text-text">{quiz.name}</td>
                      <td className="whitespace-nowrap px-6 py-4 text-gray-700">{quiz.score}</td>
                      <td className="whitespace-nowrap px-6 py-4 text-gray-700">{quiz.date}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
