...
const AdminUsersPage = () => {
    ...
    try {
      ...
      const usersData: UserWithStats[] = await Promise.all(
        querySnapshot.docs.map(async (doc) => {
          ...
          const quizzesCountSnapshot = await getCountFromServer(collection(db, 'users', doc.id, 'quiz_history'));
  ...
  const handleClearHistory = async () => {
    ...
    try {
      ...
      // Update local state
      setUsers((prev) => prev.map((u) => (u.id === userToClear.id ? { ...u, quizzesCompleted: 0 } : u)));
